...
class QLearningAlgorithm(util.RLAlgorithm):
    ...
    def incorporateFeedback(self, state, action, reward, newState):
        ...
        if newState is not None:
            Vop=max([self.getQ(newState,newAction) for newAction in self.actions(newState)])
        Qop=self.getQ(state,action)
        adjustment=-self.getStepSize()*(Qop-(reward+self.discount*Vop))
