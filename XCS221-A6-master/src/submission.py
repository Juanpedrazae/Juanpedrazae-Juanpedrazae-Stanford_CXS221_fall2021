...
class ExactInference(object):
    ...
    def elapseTime(self):
        ...
        # ### START CODE HERE ###
        newBf=util.Belief(self.belief.numRows,self.belief.numCols,value=0.0)
        for oldTile,newTile in self.transProb:
            value=self.transProb[(oldTile,newTile)]
            if value>0:
                newBf.addProb(newTile[0],newTile[1],self.belief.getProb(oldTile[0],oldTile[1])*value)
        newBf.normalize()
        self.belief=newBf
        # ### END CODE HERE ###
        ...
        for tile in self.particles:
            if tile in self.transProbDict:
                ...
                for i in range(value):
                    ...
                    z=util.weightedRandomChoice(newWeightDict)
                    addFactor=0
                    newParticles[z]+=1+addFactor
