...
class BacktrackingSearch():
    ...
    def arc_consistency_check(self, var):
        ...
        qu=[(j,var) for j in self.csp.get_neighbor_vars(var)]
        while qu:
            var_k,var_j=qu.pop(0)
            if remove_inconsistency(var_k,var_j):
                for var_l in self.csp.get_neighbor_vars(var_k):
                    if var_l!=var_k:
                        qu.append((var_l,var_k))
        # ### END CODE HERE ###
        ...
        for quarter in self.profile.quarters:
            ...
            for request in self.profile.requests:
                ...
                for courseId in request.cids:
                    ...
                    domains = list(range(self.bulletin.courses[courseId].minUnits, self.bulletin.courses[courseId].maxUnits+1)) + [0]
